...
# Actual data: http://py4e-data.dr-chuck.net/comments_1006557.xml (Sum ends with 43)
...
url = 'http://py4e-data.dr-chuck.net/comments_42.xml'  # the test link
...
print ("Retrieving", url) #Retrieving http://py4e-data.dr-chuck.net/comments_42.xml
html = urllib.request.urlopen(url, context=ctx)
data = html.read()
print("Retrieved",len(data),"characters") #Retrieved 4189 characters

tree = ET.fromstring(data)
results = tree.findall('comments/comment')
count=len(results) #Count: 50

sum=0
for result in results:
    sum += float(result.find('count').text) ## x += n, is the same x = x + n
print(count)
print(sum)
